...
function SignUpPage({ activeUser ,onSignUp}) {
        ...
        if (pass === verifyPass) {
            (async () => {
                ...
                try {
                    ...
                    //for error like 202 - user already exist
                    if(userResult.message){
                        console.Alert('Error while signing up user', userResult.message);
                        setShowInvalidSignUp(userResult.message);
                    } else {
                        //new active user create
                        const activeUser = new UserModel(userResult);
